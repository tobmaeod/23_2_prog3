...
if st.button("검토 파일 생성하기"):
    if not article:
        st.error("내용을 입력한 후, 파일을 생성하세요.")     # article이 입력되지 않았다면 오류 메시지 출력
    else:
        student_id = st.text_input("학번을 입력하세요.")        # 학생 정보 input widget
        ...
        href = f'<a href="data:application/octet-stream;base64,{b64_cr_txt}" download="{student_id}_{student_name}_article_cr.docx">MS Word 파일로 다운로드하기</a>'
